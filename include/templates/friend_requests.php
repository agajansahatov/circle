<?php 
	if($db){
		...
		if(!empty($_REQUEST['menu_list'])){
			if($_REQUEST['menu_list'] == 'came'){
			...
			if($friends1->rowcount() > 0){
				foreach ($friends1 as $friend){
					...
					$friends_list .= $HTML->make_standart_form($PDO_user, $selected_id, '&page=friends&menu_top=requests&menu_list=' . $_REQUEST['menu_list']);
				}
			}
			...
			<div class="main_body" id="main_body">
				...
				<form method="POST" action="">
					<div class="menu_top">
						...
						<li class="menu_top_selected_elem">Requests</li>
					...
					<div class="friends_list" id="friends_list">
						...
						<?php 
							if(!empty($_REQUEST['menu_list'])){
								if($_REQUEST['menu_list'] == 'came'){
									echo '<li id="list_option_selected">Received</li><a id="list_option" href="?menu_top=requests&menu_list=sent">Sent</a>';
								}else if($_REQUEST['menu_list'] == 'sent'){
									echo '<a id="list_option" href="?menu_top=requests&menu_list=came"">Received</a><li id="list_option_selected">Sent</li>';
								}
							}
							echo '<li id="search_indicator">' . ($friends1->rowcount() +  $friends2->rowcount() . ' people</li>');
