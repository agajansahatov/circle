<?php
	if($db){
		$friends1 = $db->conn->query('SELECT user2_id from friends WHERE user1_id = "' . $_SESSION['Agajan_Circle_id'] . '" and user1_block = 1');
		$friends2 = $db->conn->query('SELECT user1_id from friends WHERE user2_id = "' . $_SESSION['Agajan_Circle_id'] . '" and user2_block = 1');
		$friends_list = '';
		...
		if($friends1->rowcount() > 0){
			foreach ($friends1 as $friend){
				...
				$friends_list .= $HTML->make_standart_form($PDO_user, $selected_id, '&page=friends&menu_top=blocked');
			}
		}
			...
			<div class="main_body" id="main_body">
				...
				<form method="POST" action="">
					<div class="menu_top">
						...
						<li class="menu_top_selected_elem">Blocked users</li>
